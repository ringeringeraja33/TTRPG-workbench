...
最新资料吸收：已研究一批包含CHM、扫描件、图片、旧DOC和自动卡的参考材料，将已核查流程与出处写入技能；使用这些流程无需访问维护者的原件或私有索引。新增[全文入口与自动卡审计](corpus-use.md)、[系统适配与明确缺口](systems/corpus-adapters.md)、[模组与展示材料流程](corpus-preparation.md)。提取成功与条款逐条核验分别记录；不宣称93个文件的全部规则已完成审校或自动结算。以下历史表中的“CHM未解”和TOC恢复“未读”已由本轮对应主题更新取代。
