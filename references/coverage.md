...
| [Local dicebot and character registration](dicebot-cards.md) | partial | supported | supported | partial | partial | Table/player-scoped persistent cards, aliases, atomic numeric updates, nickname previews, export and actual CoC7 checks with retry-safe dice. Local dialect only; no external bot connection, private-channel transport or automatic character legality certification. CoC7 base attribute batches support raw-dice evidence, explicit selection and preserved originals; age changes, derived values and full creation legality remain guided steps. |
| [Local Dice! adaptation and table tools](dice-local.md) | partial | partial | partial | partial | partial | Bounded Dice! RD adaptation with cards, teams, logs, opt-in exact replies, local join welcome, owner timers with foreground wait/ack, and attributed weighted/nested JSON decks. Native DLL, historical script languages, full datasets and external services remain incomplete. |
