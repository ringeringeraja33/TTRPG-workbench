...
Use this for 骰娘, 录卡, 查卡, 改卡, 切卡, `.st`, `.ra`, and nickname or bot-command requests. Use the bundled local adapter without access to the maintainer's books. Answer in the user's language. A character's recorded values are not proof of legal creation or advancement.

## Choose the execution mode

**Local Workbench:** execute `scripts/dicebot.py`. This is a documented local dialect, `workbench-local-v1`, with persistent cards and actual OS-random dice. It does not connect to QQ, alter group nicknames or implement every external dice bot.

**External bot:** identify the actual bot and version from information supplied by the user or its current official help. Provide copyable commands matching that dialect and a readback checklist. Do not assume tower, Dice!, SealDice, mobile bots or historical aliases use identical syntax. Sending messages, changing a live group card, binding another person's card or uploading logs needs the user's explicit request for that action; generating command text does not send it.
...
2. Read the user's simplified-card text or selected spreadsheet fields. Use `sheet_audit.py` when formulas or cached values are involved. Keep background secrets, contacts and GM notes out of the bot import string. Show uncertain OCR values for review; do not guess them.
...
6. Read the whole card, then spot-check aliases and important fields. Export only recorded numeric fields. When using an external bot, require its returned values before saying the import succeeded; generated commands alone are not evidence of registration.
...
| `.st export` | Produce a local-dialect import command; verify external compatibility before sending |
| `.nn阿达` | Rename the local card |
| `.sn` / `.sn阿达` | Preview a nickname from card name and recorded HP/SAN, or the supplied text; no group change |
...
`.sc`, `.en`, bot-specific `.st name-...`, deletion, log upload and group administration are not supported local commands. For SAN/development use the sourced `rules_math.py` procedures and session ledger; generating a command for an external bot requires its actual syntax and configured house rules. Do not silently treat an unsupported command as a successful operation.
...
Table/player fields isolate records for a trusted local operator; they are not an authentication service. A deployed bot must authenticate the sender and enforce ownership before mapping to these fields. Never accept a chat user's claim to another owner's ID. Back up the database through SQLite's backup facility or after closing all writers; copying a live database alone may omit journaled data.

## Source and interpretation

yy, *Welcome to Call of Cthulhu RPG / 教你如何快速成为一个合格的调查员*, sections “关于车卡录卡的那些事” and “如何正确的调戏骰娘”; user-supplied text and screenshot, version/date unstated. The screenshot demonstrates `.st show` and `.st show灵感`. This is a community tutorial and command-usage reference, not an official rules authority or evidence of current bot availability. No private document URL or contact details are needed in citations.

The tutorial's penalty-die prose incorrectly says to choose the smaller result although its example chooses61 over11. The local adapter chooses the larger complete percentile result for a penalty and the smaller for a bonus. It treats00 with units0 as100: candidates100 and10 produce10 with a bonus,100 with a penalty. It does not adopt suggested 80/60 caps or point-buy budgets as universal CoC rules. Rule outcomes use the existing [CoC7 procedures](systems/coc7-procedures.md); external bot lifecycle claims in the tutorial are historical and unverified.
...
Rule basis: the bundled synthesis of *Call of Cthulhu*, seventh edition core rules, character creation chapter; Chinese core translation v1.2.1, PDF pages23–26. Command inspiration: yy's tutorial cited above. No fresh original-source audit or external bot compatibility is claimed for this extension.
